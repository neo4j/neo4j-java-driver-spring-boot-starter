...
class TestSliceTest {
	...
	public void sessionFactoryShouldUseOGMBoltDriver() {
		assertThat(sessionFactory.getDriver()).isInstanceOf(BoltDriver.class);

		// We cannot assert equality without reflection, will be possible in OGM 3.2 with unwrap
		assertThat(ReflectionTestUtils.getField(sessionFactory.getDriver(), "boltDriver")).isSameAs(this.driver);
	}
